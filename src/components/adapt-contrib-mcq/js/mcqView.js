define([
  ...
  var McqView = QuestionView.extend({
    ...
    events: {
      ...
      'keyup .js-item-label': 'onKeyPress'
    },
    ...
    updateSelection: function() {
      ...
      this.model.getChildren().each(function(itemModel) {
        ...
        var $itemLabel = this.$('.js-item-label').filter('[data-adapt-index="' + index + '"]');
        //console.log(isSelected ? ariaLabels.selectedAnswer : ariaLabels.unselectedAnswer);
        var newAriaLabel =  (isSelected ? ariaLabels.selectedAnswer : ariaLabels.unselectedAnswer) + ". " + $.a11y_normalize(itemModel.get("text"));
        $itemInput.attr('aria-label', newAriaLabel);
        $itemLabel.attr('aria-label', newAriaLabel);
        //console.log(isSelected, newAriaLabel);
      ...
      this.model.getChildren().each(function(itemModel) {
        ...
        if (isInteractive || !canShowMarking) {
         ...
         // $itemInput.attr('aria-label', $.a11y_normalize(itemModel.get("text")));
          ...
          //console.log('updateMarking', $itemInput.attr('aria-label'));
          return;
        }
